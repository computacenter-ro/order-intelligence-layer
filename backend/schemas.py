...
class ChatResponse(BaseModel):
    ...
    # Minted per answer so the dashboard can rate THIS reply (POST /chat/feedback).
    # Generated by the backend, not the AI service: the backend owns the votes.
    answer_id: str = ""
...
class ChatFeedbackRequest(BaseModel):
    ...
    ``answer_id`` comes from the ``ChatResponse`` being rated, so a vote can never
    be attributed to the wrong reply. Re-voting the same answer replaces the
    previous vote (the row is keyed by ``answer_id``), which is what lets an agent
    change their mind without inflating the tally.
    """

    answer_id: str
    # True = thumbs up. A bool rather than +1/-1 so the wire contract cannot carry
    # a meaningless 0; the DB stores the signed form.
    liked: bool
    query: str = ""
    # Cited record ids IN THE ORDER SHOWN — position drives rank-weighted
    # attribution, so a reordered list would mis-credit the sources.
    record_ids: list[str] = []
...
class ChatFeedbackResponse(BaseModel):
    recorded: bool
    # Echoed back so the UI can reflect the stored state rather than assume its
    # optimistic update won.
    liked: bool
