...
The order engine takes TWO turns under the five-hop ping-pong. Its blocks:

  * ``init`` — phase 1, FIRST turn: consume ``order.init``, assemble the
    DEFAULT ORDER DATA, persist **nothing**, publish ``order_data_ready`` back
    to Inbound. No ids are minted here — the order does not exist yet.

  * ``create`` — phase 1, SECOND turn: consume ``order.approval``, persist the
    cart header to BM DB (order INACTIVE), **mint orderId + cartHeaderId into
    ctx**. This is the ONLY block that fills the order ids. Failure variant
    (``fail_at=create``, scenario 5): BM-DB timeout ×3 → fatal abort, chain
    stops. There is no creation-response publish anymore — the only return hop
    after creation is the terminal ``order_created`` close (inbound.py).

  * ``enrich_<sat>_call`` / ``enrich_<sat>_resp`` — the Feign-style client
    ``--->`` / ``<---`` logs that bracket each post-creation satellite call,
    plus the order-engine-side processing filler (org hierarchy + object attrs
    + contract/fee/cart lines before the FIRST satellite; pricing after SPT;
    PVC/rebate around RSM). The satellite's own server block runs in between
    (its own module).

    Satellite ORDER comes from ``ENRICH_SATELLITES`` (SPT → RSM → Validator →
    Checker; SPT/RSM's position is INFERRED, Validator→Avalara→Checker is the
    documented auto-approval rule order — see shared/scenarios.py) and is
    ...
    Settings/JAM/SOLR are NOT here anymore: they are Inbound's pre-creation
    calls (inbound.py).

  * ``dispatch`` — publish the checked order to ``order.create.sap``.

Id discipline: ``init`` and ``create`` are phase 1 (eventId only) — ``create``
stays eventId-only in its FIELDS right through minting, while its message
*text* prints the minted ids. That text is the correlation join the stitcher
mines (backend/stitching.py); changing those strings means changing the mining
patterns and their tests in the same commit. Every enrich/dispatch block is
phase 2 (both order ids, never eventId).
"""
...
# Loggers (verbatim from the reference dataset; OrderInitListener and
# DefaultOrderDataService are new with the five-hop realignment — the engine
# never had a defaults-only first turn before).
_LOG_INIT_LISTENER = "c.c.orderengine.listener.OrderInitListener"
_LOG_DEFAULTS = "c.c.orderengine.service.DefaultOrderDataService"
...
# emits its own lines) with no order-engine client `--->`/`<---` log. The
# validator likewise runs without a client bracket — its `validate` block emits
# everything, including the OE-identity abort on failure. Settings/JAM/SOLR
# clients moved to inbound.py with the calls themselves.
_CLIENT_LOGGER = {
...
# Satellites the engine invokes in-process, with no Feign client log of its own.
_NO_CLIENT_LOG = ("checker", "validator")
...
# --- init (phase 1) — the FIRST turn ------------------------------------------
@register("order_engine", "init")
async def init(baton: Baton, emit: EmitFn) -> bool:
    """First turn: consume order.init, assemble default order data, persist
    NOTHING, publish order_data_ready. The order — and its ids — do not exist
    yet; every line here is eventId-only."""
    ctx = baton.ctx
    ids = phase1_ids(ctx)

    await emit_line(
        emit, _PROF, logger=_LOG_INIT_LISTENER, level="INFO", thread=_CREATE_THREAD,
        message=f"Received order.init for event {ctx.eventId}",
        ids=ids,
    )
    await emit_line(
        emit, _PROF, logger=_LOG_DEFAULTS, level="INFO", thread=_CREATE_THREAD,
        message=(
            f"Assembling default order data for event {ctx.eventId}, "
            f"account {ctx.accountNumber} ({ctx.country})"
        ),
        ids=ids,
    )
    await emit_line(
        emit, _PROF, logger=_LOG_DEFAULTS, level="DEBUG", thread=_CREATE_THREAD,
        message=f"Default order data assembled for event {ctx.eventId}; nothing persisted",
        ids=ids,
    )
    await emit_line(
        emit, _PROF, logger=_LOG_PUBLISHER, level="INFO", thread=_CREATE_THREAD,
        message=f"Published order_data_ready for event {ctx.eventId}",
        ids=ids,
    )
    return True  # forward to inbound's enrichment leg


# --- create (phase 1) — the SECOND turn ---------------------------------------
@register("order_engine", "create")
async def create(baton: Baton, emit: EmitFn) -> bool:
    """Second turn: consume order.approval, persist, mint ids into ctx (or fail)."""
    ctx = baton.ctx
    ids = phase1_ids(ctx)
    ...
    # THE JOIN — these two message texts are load-bearing for correlation: they
    # carry eventId as a FIELD and the minted order ids in their TEXT, which is
    # the only thing that ties the two id families together
    # (backend/stitching.py mines them). Change them only together with the
    # mining patterns and their tests.
    await emit_line(
        emit, _PROF, logger=_LOG_CREATION, level="INFO", thread=_CREATE_THREAD,
        message=f"Created cart header {ctx.cartHeaderId} with status INACTIVE",
    ...
    return True  # forward to track_trace/register
...
async def _create_failure(emit: EmitFn, ctx: BatonContext) -> bool:
    """BM-DB timeout ×3 → fatal abort. No ids ever.

    Scenario 5: creation itself fails, so the order ids are never minted — the
    journey lives and dies eventId-only. Nothing is published back to Inbound:
    under the five-hop shape the only post-creation return hop is the terminal
    order_created, which a failed creation never sends (Inbound therefore logs
    no wrap-up, and the pipeline trail warns nobody for this outcome).
    """
    ids = phase1_ids(ctx)
    ...
    # LOAD-BEARING: backend/journeys.py classifies ORDER_CREATION_FAILED on
    # "order creation failed for event".
    await emit_line(
        emit, _PROF, logger=_LOG_CREATION, level="ERROR", thread=_CREATE_THREAD,
...
async def _orchestration_preamble(baton: Baton, emit: EmitFn) -> None:
    ...
    ObjectAttributeService, then the contract/fee/cart processing filler —
    emitted once, right before the FIRST satellite's call line (SPT, per
    ENRICH_SATELLITES). The internal-contract "No internal contracts found"
    WARN lives here (it used to follow the Settings response when Settings was
    an engine call); it is benign and sits in the AI service's suppression
    list — keep the wording.
    """
    ctx = baton.ctx
    ids = phase2_ids(ctx)
    ...
    lines = [
        ...
        (_LOG_INTERNAL_CONTRACT, "DEBUG", f"Attempting to retrieve internal contracts for sales org: {org}"),
        (_LOG_INTERNAL_CONTRACT, "WARN", f"No internal contracts found for sales org: {org}"),
        (_LOG_FEE_CONFIG, "DEBUG", f"Extracting FeeConfigs by countryCode {ctx.country}"),
        (_LOG_CART_BLOCKING, "DEBUG", f"Get cart blocking and grouping items for header id {ctx.cartHeaderId}"),
        (_LOG_TEXTS_OTHERS, "INFO", f"Extracting cart items universal attributes for line udfs by {ctx.cartHeaderId}"),
        (_LOG_FEE_ITEM, "DEBUG", f"Get cart fees for header id {ctx.cartHeaderId}"),
        (_LOG_CART_SOURCING, "DEBUG", f"Get cart sourcing items for cart header id {ctx.cartHeaderId}"),
    ]
...
def _make_enrich_call(sat: str):
    ...
    async def _call(baton: Baton, emit: EmitFn) -> bool:
        ...
        # Checker and validator have no order-engine client call line — each
        # satellite's own block emits everything. Nothing to emit here.
        if sat in _NO_CLIENT_LOG:
            return True
...
def _make_enrich_resp(sat: str):
    ...
    async def _resp(baton: Baton, emit: EmitFn) -> bool:
        ...
        elif sat in _NO_CLIENT_LOG:
            # Checker's / validator's own blocks emit everything; nothing here.
            pass
...
# inserts its trio conditionally (between Validator and Checker) rather than
# listing it in ENRICH_SATELLITES — but the handlers must still be registered,
# or a US chain would dispatch to a missing block.
...
async def dispatch(baton: Baton, emit: EmitFn) -> bool:
    """Publish the checked order to order.create.sap (phase 2)."""
    ctx = baton.ctx
    await emit_line(
        emit, _PROF, logger=_LOG_PUBLISHER, level="INFO", thread=_worker_thread(baton),
        message=(
            f"Published order {ctx.orderId} to queue order.create.sap "
