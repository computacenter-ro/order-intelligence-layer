  ...
  return (
    ...
    >
      <svg
        ...
        height="100%"
        role="img"
        aria-label="Architecture diagram of the simulated order pipeline. Orders arrive from B2B and Salesforce through SAP BTP into the Inbound service, which calls Settings, JAM and SOLR before the order exists, then ping-pongs with the Order Engine over RabbitMQ: order init, order data ready, order approval. The Order Engine persists to the BM database, registers Track and Trace, calls SPT, RSM, Validator, Avalara and Checker, submits through Outbound OSW to SAP Fulfilment, and answers order created back to Inbound, where the flow ends."
      >
        <defs>
