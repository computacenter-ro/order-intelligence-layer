# Order Journey Tracker — Chatbot Context

**Part 1** is universal: how to answer, and how orders are identified across services. It
applies to every service and must not be repeated in per-service files.
**Part 2** documents one service, `order-engine`.

Every fact in Part 2 was verified against the source. Where something could not be
...
If a service file conflicts with this Part 1, or two service files conflict, **report the
...
| 2 | Order validation | Order Validator | `TODO` | rules engine |
| 3 | Pricing & rebates | SPT, RSM | `TODO` | enrich the order before submit |
...
| 6 | **Order creation & submission** | **order-engine** | Part 2 below | the hub |
...
| Service | Status |
|---|---|
| order-engine | complete (Part 2) |
| all others | `TODO` — copy `_TEMPLATE-new-service.md` |
...
Both Parts number their sections from 1, so note: **every `§` reference below refers to
Part 2.** When citing Part 1, name it — "Part 1, answering rules".
